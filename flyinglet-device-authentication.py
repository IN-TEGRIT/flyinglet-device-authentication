    ...
    class InstallIntegrit:
        ...
        def verification(self, stdscr):
            ...
            while retry_count < max_retry_count:
                ...
                # Print prompt to screen
                stdscr.addstr(2, 0, 'Enter SECRET_KEY: ')
                ...
                while True:
                    ...
                    if c == ord('\n'):
                        ...
                        if len(secretkey.rstrip()) > 0:
                            ...
                            stdscr.addstr(2, len('Enter SECRET_KEY: ') + len(secretkey), ' ')  # 기존의 문자를 공백으로 덮어씀
                            stdscr.move(2, len('Enter SECRET_KEY: ') + len(secretkey))
                        else:
                            secretkey = ''
                            stdscr.move(2, len('Enter SECRET_KEY: ') + len(secretkey))
                    else:
                        ...
                        stdscr.addch(2, len('Enter SECRET_KEY: ') + len(secretkey) - 1, '*')
    
                # stdscr.addstr(6, 0, f'Enter robot ID: {response}')
    
                response = install_integrit.signin_device(fcode, secretkey)
    
                # 아이디 유무 확인
                if response.status_code == 200:
                    stdscr.addstr(4, 0, 'verification successfully completed')
                    # Get MAC address
                        ...
                        while True:
                            ...
                            if c == ord('\n'):
                                ...
                                if len(answer.rstrip()) > 0:
                                    answer = answer[:-1]
                                    stdscr.addstr(6, len(question) + len(answer), ' ')  # 기존의 문자를 공백으로 덮어씀
                                    stdscr.move(6, len(question) + len(answer))
                                else:
                                    answer = ''
                                    stdscr.move(6, len(question) + len(answer))
                            else:
                                ...
                                # Print SECRET_KEY to screen
                                stdscr.addch(6, len(question) + len(answer) - 1, c)
                    ...
                    # Print newline character
                    stdscr.addstr(8, 0, "The task has been completed. The program will now exit. Please press any key.")
                    ...
                    stdscr.addstr(8, 0, 'Incorrect F-CODE or SECRET_KEY. Retry {}/{}'.format(retry_count, max_retry_count))
            ...
            if retry_count == max_retry_count:
                stdscr.addstr(8, 0, 'Maximum retry count reached. Please try again later.')
                stdscr.refresh()
                stdscr.getch()
